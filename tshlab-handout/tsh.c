...
eval(char *cmdline) {
    ...
    Sigemptyset(&sigmask);    //block the SIGCHLD SIGINT and SIGTSTP
    ...
    if (tok.builtins == BUILTIN_QUIT)
        // if the buildtin command is 'quit', we should quit the tsh.
    {
        //close file
        ...
        // call listjobs() to print jobs bg
    {
        listjobs(job_list, output);
    } else if (tok.builtins == BUILTIN_BG)
        // change bg(stopped) to bg(running)
    {
        bgfg(tok.argv[1], 1);
    ...
    } else  //not the builtin command
    {
        ...
        if (!bg) {
            //int status;
            //pid_t quit_fg;
            //if ((quit_fg=Waitpid(pid,&status,0))<0) printf("Oh, No!\n");
            //deletejob(job_list,quit_fg);
            Sigprocmask(SIG_BLOCK, &sigmask, &prev);//block the signal
            while (pid == fgpid(job_list)) Sigsuspend(&prev);//safely use pause();
            Sigprocmask(SIG_SETMASK, &prev, NULL);//unblock the signal
            /* Test !!! Don't forget*/
            //printf("-----Test say: %d -----\n",quit_fg);
        } else {
            printf("[%d] (%d) %s\n", pid2jid(pid), pid, cmdline);
...
void bgfg(char *s, int bg) {
    ...
    //when it be cleared, you will get trouble---race!!!!!
    if (bg) {
        job->state = BG;
...
sigchld_handler(int sig) {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        if (WIFEXITED(status))//the child exit normally

...
#define is_front_used(node) (((*node)>>1)&1)
#define max(x,y) ((x>y)?x:y)
typedef unsigned int UI;
static inline UI *pointer(UI*node) {return (UI *)(mem_heap_lo()+(*node));}
...
#define LIST_SIZE 4
#define BST_SIZE 5
#define MAX_IN_LIST 24
#define MIN_IN_LIST 16
static UI*list_head,*list_tail,*BST_root,*begin;
static int debug=0,if_exit=1;
...
int mm_init(void) {
	if ((unsigned long long)mem_heap_hi()&7) mem_sbrk(4);	//for alignment
	else mem_sbrk(8);
...
//insert node into BST
{
	...
	{
		BST_root=node;
		init_node_BST(node,*node,mem_heap_lo());
		return;
	}
	//printf("in %lld\n",(long long)r);
	UI rsize=getsize(r);
	...
	if (*(r+t)) insert_BST(*(r+t)+mem_heap_lo(),node,size);
	else
	{
...
//delete r from BST
{
	...
	{
		UI*FA=fa(r);
		if (FA!=mem_heap_lo()) *(FA+isson(FA,r)+2)=*(r+2);
		UI*C=lc(r);
		*(C+1)=*(r+1);
		replace=C;
	}
	...
	{
		UI*FA=fa(r);
		if (FA!=mem_heap_lo()) *(FA+isson(FA,r)+2)=*(r+3);
		UI*C=rc(r);
		*(C+1)=*(r+1);
		replace=C;
	}
	...
	for(;in_heap(p);p+=getsize(p)/4)
	{
		...
		else
		{
			...
			else 
			{
				...
				if (BST_root==p&&fa(p)!=mem_heap_lo())
				{

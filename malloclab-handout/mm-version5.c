...
#define remain_heap (*(free_head+5))
#define buffer (1<<10)
...
static int in_heap(const void *p) {
    return p <= mem_heap_hi()-remain_heap && p >= mem_heap_lo();
}
...
static void * SBRK(unsigned int size)
{
	if (size>remain_heap) 
	{
		unsigned int dlt=max(size-remain_heap,buffer);
		//printf("%lld\n",dlt);
		mem_sbrk(dlt);
		remain_heap+=dlt;
	}
	void * ans=mem_heap_hi()-remain_heap+1;
	remain_heap-=size;
	//printf("%lld %lld %lld\n",(long long)size,(long long)remain_heap,(long long)mem_heap_hi());
	return ans;
}

...
void *malloc (size_t size) {
	unsigned int newsize=REMAIN_SIZE*4+ALIGN(size);
...
void free (void *ptr) {
	//fprintf(stderr,"%lld\n",(long long)ptr);
	if (ptr==NULL) return;
	unsigned int*Ptr=((unsigned int*)ptr)-REMAIN_SIZE;
	delete(Ptr);
	mm_checkheap(187);
    if(!ptr) return;
}
...
void *realloc(void *Oldptr, size_t size) {
	...
	{
		oldptr=oldptr-REMAIN_SIZE;
		size=REMAIN_SIZE*4+ALIGN(size);
		...
		if (dlt>0)
		{
			...
			if (in_heap(next)&&!is_used(next)&&(long long)getsize(next)>=dlt)
			{
				...
				mm_checkheap(224);
				return oldptr+REMAIN_SIZE;
			}
			else
			{
				unsigned int *new=malloc(size-REMAIN_SIZE*4);
				new-=REMAIN_SIZE;
				unsigned int len=getsize(oldptr)/4;
				//printf("%d\n",len);
				for(unsigned int i=REMAIN_SIZE;i<len;i++) *(new+i)=*(oldptr+i);
				free(oldptr+REMAIN_SIZE);
				//printf("yes?\n");
				mm_checkheap(234);
				return new+REMAIN_SIZE;
			}
		}
		else
		{
...
void *calloc (size_t nmemb, size_t size) {
	...
	for(unsigned int i=REMAIN_SIZE;i<len;i++) *(ans+i)=0;
	mm_checkheap(263);
    return ans+REMAIN_SIZE;
}
